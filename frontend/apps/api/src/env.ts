...
const envSchema = z.object({
  ...
  AWS_BACKEND_API_URL: z.string().url().optional(),
  USERS_TABLE: z.string().min(1).optional(),
...
const parsed = envSchema.safeParse({
  ...
  AWS_BACKEND_API_URL: emptyToUndefined(process.env.AWS_BACKEND_API_URL),
  USERS_TABLE: emptyToUndefined(process.env.USERS_TABLE),
  TOTP_ENCRYPTION_KEY: emptyToUndefined(process.env.TOTP_ENCRYPTION_KEY),
